...
enrich_candidates), and db/prospects.py (dedup_filter, insert_enriched) into
the teammate-facing vertical slice per 02-UI-SPEC.md.
...
insert_enriched) is wrapped in try/except so an unexpected exception renders
a plain-language st.error banner instead of a Streamlit traceback, matching
...
NEVER log or print the api_key (T-02-04).
"""
...
from db.prospects import dedup_filter, insert_enriched
...
if st.button(
    ...
    # Re-searching always discards any prior search/enrichment result (no
    # separate "New Search" control needed per UI-SPEC).
    st.session_state.enrich_result = None
