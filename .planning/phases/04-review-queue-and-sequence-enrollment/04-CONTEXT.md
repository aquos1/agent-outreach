...
- **D-19 (conflict resolution, post-planning):** D-14's fix creates real tension with D-04 ("no stale previews — what you see always matches what will send"): only the opening line actually travels to Apollo; the static paragraph text stays wherever it is in Apollo's own sequence editor. Editing the template's paragraphs in the Review Queue after this phase updates the *preview* only — it has no effect on what Apollo sends unless the teammate also manually pastes matching text into Apollo's sequence editor. Explicitly accepted rather than resolved: ship with a clear warning caption under the template editor and an explicit acceptance step in the 04-05 human-verify checkpoint, rather than either (a) making the paragraph text read-only, or (b) researching whether Apollo's API supports programmatic sequence-body updates (deferred — real scope, not answerable without new research, would delay execution).
